...
class ViaStreamHandler:
    ...
    def _trigger_query(self, req_info: RequestInfo, start_time: float = None):
        ...
        # Check for twelve-labs multi-video scenario
        model_info = self.get_models_info()
        if model_info.id == "twelve-labs" and len(req_info.assets) > 1:
            # For twelve-labs with multiple videos, process each individually and combine results
            logger.info(f"Detected twelve-labs multi-video request with {len(req_info.assets)} videos")
            
            # Extract video IDs for twelve-labs multi-video processing
            video_ids = [asset.asset_id for asset in req_info.assets if asset.asset_id]
            
            # Validate video_ids
            if not video_ids:
                logger.error("No valid asset IDs found for twelve-labs multi-video processing")
                req_info.status = RequestInfo.Status.FAILED
                return
            
            if len(video_ids) != len(req_info.assets):
                logger.warning(f"Some assets missing asset_id: {len(req_info.assets)} assets but only {len(video_ids)} valid IDs")
            
            # Validate asset paths
            valid_assets = [asset for asset in req_info.assets if asset.path and os.path.exists(asset.path)]
            if len(valid_assets) != len(req_info.assets):
                logger.warning(f"Some asset paths invalid: {len(req_info.assets)} assets but only {len(valid_assets)} valid paths")
                if not valid_assets:
                    logger.error("No valid asset paths found for twelve-labs multi-video processing")
                    req_info.status = RequestInfo.Status.FAILED
                    return
            
            # Create a single chunk for twelve-labs to process
            chunk = ChunkInfo()
            chunk.chunkIdx = 0
            chunk.is_first = True
            chunk.is_last = True
            chunk.file = ";".join([asset.path for asset in valid_assets])  # Use validated assets
            chunk.start_pts = 0
            chunk.end_pts = 0
            chunk.pts_offset_ns = 0
            chunk.streamId = valid_assets[0].asset_id
            chunk.cv_metadata_json_file = req_info.cv_metadata_json_file
            
            # Set proper NTP timestamps for VSS aggregation sorting
            current_time = time.time()
            chunk.start_ntp = get_timestamp_str(current_time)
            chunk.end_ntp = get_timestamp_str(current_time)
            chunk.start_ntp_float = current_time
            chunk.end_ntp_float = current_time
            
            # Process the multi-video chunk directly
            req_info.chunk_count = 1
            
            # Add video_ids to vlm_request_params for twelve-labs
            multi_video_params = req_info.vlm_request_params
            if not multi_video_params:
                # Create VlmRequestParams if None
                multi_video_params = VlmRequestParams()
                multi_video_params.vlm_generation_config = {"video_ids": video_ids}
            elif multi_video_params.vlm_generation_config:
                multi_video_params.vlm_generation_config["video_ids"] = video_ids
            else:
                multi_video_params.vlm_generation_config = {"video_ids": video_ids}
            ...
            self._vlm_pipeline.enqueue_chunk(
                ...
                req_info.request_id,
                None,  # video_codec not needed for twelve-labs
            )
            return
